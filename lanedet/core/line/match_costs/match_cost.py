    ...
    def __call__(self, line_pred, gt_lines):
        ...
        Args:
            ...
            gt_lines (Tensor): Ground truth boxes with normalized
                coordinates (x1,x2,...,xN), N= num_points. Shape (num_query, num_points).
        Returns:
...
class LineIoUCost:
    ...
    def __call__(self, lines, gt_lines, length=1e-5):
        """
        Args:
            lines (Tensor): Predicted boxes with unnormalized coordinates
                (x1, x2, x3,...,xN), N=num_points. Shape (num_query, num_points).
            gt_lines (Tensor): Ground truth boxes with unnormalized
                coordinates (x1, x2, x3,...,xN), N=num_points. Shape (num_gt, num_points).
            length:(int | float): extended radius,default=1e-5
        Returns:
            torch.Tensor: line_iou_cost value with weight
        """
        # iou_cost: [num_bboxes, num_gt]
        iou_cost = line_overlaps(
            lines, gt_lines, length, is_aligned=False)
...
class ClassificationCost:
    ...
    def __call__(self, cls_pred, gt_labels):
        ...
        a = gt_labels.clone().detach().long()
        cls_cost = -cls_score[:, a]
        return cls_cost * self.weight
